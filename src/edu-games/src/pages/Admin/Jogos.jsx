...
export default function Jogos() {
    ...
    async function carregarDados() {
        try {
            ...
            setJogos(Array.isArray(listaJogos) ? listaJogos : []);
            setCategorias(Array.isArray(listaCats) ? listaCats : []);
            setEmpresas(Array.isArray(listaEmpresas) ? listaEmpresas : []);
        } catch (error) {
            console.error("Erro crítico ao carregar dados dos jogos:", error);
        } finally {
            setLoading(false);
        }
    }
    ...
    if (loading) {
        return (
            <div className="admin-container">
                <HeaderAdmin />
                <div className="admin-content-body" style={{ display: 'flex', justifyContent: 'center', paddingTop: '100px' }}>
                    <p style={{ color: 'white', fontSize: '1.2rem' }}>Carregando Jogos...</p>
                </div>
            </div>
        );
    }
